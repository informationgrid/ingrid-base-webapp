...
public class IngridIndexSearcher implements ISearcher, IDetailer, IConfigurable {
	...
	@Autowired
	public IngridIndexSearcher(ILuceneSearcher luceneSearcher,
			List<IQueryParser> queryParsers) {
		_luceneSearcher = luceneSearcher;
		_queryParsers = queryParsers;
	}

	public IngridHits search(IngridQuery ingridQuery, int start, int length)
			throws Exception {
		...
		LOG.debug("found hits: " + topDocs.scoreDocs.length + "/"
				+ topDocs.totalHits);
		...
		IngridHits ingridHits = new IngridHits(_plugId, topDocs.totalHits,
				ingridHitArray, true);
		return ingridHits;
	}
	...
	public IngridHitDetail getDetail(IngridHit ingridHit,
		...
		Map<String, Fieldable[]> details = _luceneSearcher.getDetails(docId,
				new String[] { "title", "summary" });
		String title = getValue("title", details);
		String summary = getValue("summary", details);

		IngridHitDetail ingridHitDetail = new IngridHitDetail(ingridHit, title,
				summary);
	...
	public IngridHitDetail[] getDetails(IngridHit[] ingridHits,
			IngridQuery ingridQuery, String[] fields) throws Exception {
